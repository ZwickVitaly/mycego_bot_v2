...
from keyboards.inline_keyboards.buttons import (
...
async def marketplaces_keyboard(marketplaces: list[str]):
    ...
    for mp in marketplaces:
        mp_button = InlineKeyboardButton(
            text=str(mp), callback_data=f"marketplace_{mp}"
        )
...
async def deliveries_keyboard(deliveries: dict[int, str]):
    ...
    for d_id, d in deliveries.items():
        delivery_button = InlineKeyboardButton(
            text=f"{d[:17] + '...' if len(d) > 17 else d}",
...
async def delivery_products_keyboard(products: dict[int, dict]):
    ...
    for p_id, prod_info in products.items():
        product_button = InlineKeyboardButton(
            text=f"{prod_info.get('order')}", callback_data=f"delivery_product_{p_id}"
        )
...
async def delivery_product_works_keyboard(
    available_works: dict[int, str], confirm=None
):
    keyboard = InlineKeyboardBuilder()
    keyboard.max_width = 1
    for standard_id, name in available_works.items():
        work_button = InlineKeyboardButton(
            text=f"{name}", callback_data=f"delivery_work_{standard_id}"
        )
...
async def send_delivery_keyboard():
    keyboard = InlineKeyboardBuilder()
    keyboard.max_width = 1
    keyboard.row(send_inline_button)
    keyboard.row(cancel_inline_button)
