...
from db import User, async_session, Chat
            ...
            if users_statuses and isinstance(users_statuses, list):
                ...
                async with async_session() as session:
                    async with session.begin():
                        q1 = await session.execute(select(User).where(User.id.in_(fired_ids)))
                    ...
                    async with session.begin():
                        q3 = await session.execute(select(Chat).where(Chat.admin == True))
                        chats = list(q3.scalars())
                    if chats and banned_ids:
                        logger.info("Есть чаты, где бот - админ, и пользователи, которых нужно кикнуть.")
                        for chat in chats:
                            for banned in banned_ids:
                                try:
                                    await bot.ban_chat_member(chat.id, banned, until_date=timedelta(seconds=35))
                                except TelegramBadRequest as e:
                                    logger.error(f"Нельзя удалить пользователя {banned} из чата {chat.id}. Причина: {e}")
                        async with session.begin():
                            await session.execute(delete(User).where(User.id.in_(banned_ids)))
                    else:
                        logger.info("Не найдены чаты, где бот админ, или пользователи, которых нужно забанить")
            else:
